...
const CAT_ICONS = {
  ...
  legal: (color) => (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
      <path d="M12 3L4 7v5c0 4.5 3.5 8.2 8 9 4.5-.8 8-4.5 8-9V7L12 3z" stroke={color} strokeWidth="1.8" fill="none" strokeLinejoin="round"/>
      <path d="M9 12l2 2 4-4" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  contract: (color) => (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8L14 2z" stroke={color} strokeWidth="1.8" fill="none" strokeLinejoin="round"/>
  ...
  benefits: (color) => (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
      <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" stroke={color} strokeWidth="1.8" fill="none" strokeLinejoin="round"/>
  ...
  employment: (color) => (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
      ...
      <path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2" stroke={color} strokeWidth="1.8" strokeLinejoin="round"/>
...
const CATS = [
  { id:"debt",       label:"Debt & Bills",       color:"#E8574A", hint:"e.g. I've received a final demand letter and I don't know what to do…" },
  { id:"legal",      label:"Legal Letters",       color:"#8B7EE8", hint:"e.g. I've been served a court claim form and I've never dealt with this before…" },
  { id:"contract",   label:"Contracts",           color:"#E9A84C", hint:"e.g. My landlord wants me to sign a new agreement with clauses I don't understand…" },
  { id:"tax",        label:"Tax & Revenue",       color:"#3EBF8A", hint:"e.g. I received a tax investigation notice and I'm worried what happens next…" },
  { id:"benefits",   label:"Benefits & Housing",  color:"#4BA3E8", hint:"e.g. My benefits have been stopped without explanation and I'm struggling to pay rent…" },
  { id:"employment", label:"Employment",          color:"#E87B4B", hint:"e.g. My employer is asking me to sign a settlement agreement after making me redundant…" },
];
...
/* ═══════════════════════════════════════════════════════
   VOICE INPUT
═══════════════════════════════════════════════════════ */
function VoiceInput({ onTranscript, disabled, countryCode }) {
  ...
  function start() {
    ...
    function createAndStart() {
      ...
      const rec = new SR();
      const langMap = {
        gb:"en-GB", us:"en-US", au:"en-AU", ca:"en-CA", ie:"en-IE", nz:"en-NZ",
        za:"en-ZA", ng:"en-NG", ke:"en-KE", gh:"en-GH", ph:"en-PH", sg:"en-SG",
        hk:"zh-HK", in:"hi-IN", my:"ms-MY", pk:"ur-PK", bd:"bn-BD", jp:"ja-JP",
        de:"de-DE", fr:"fr-FR", es:"es-ES", nl:"nl-NL", se:"sv-SE", no:"nb-NO",
        ch:"de-CH", pt:"pt-PT", be:"nl-BE", mx:"es-MX", br:"pt-BR",
        ae:"ar-AE", sa:"ar-SA", qa:"ar-QA", bh:"ar-BH", kw:"ar-KW", om:"ar-OM", eg:"ar-EG",
      };
      rec.lang = langMap[countryCode] || "en-GB";
...
function FollowUpChat({ catId, situation, result, chatHistory, onNewMessage, countryCode }) {
  ...
  const country = getCountryConfig(countryCode);
  const isUK = countryCode === "gb" || countryCode === "ie";
  const isUS = countryCode === "us" || countryCode === "ca";
  const suggestions = [
    "What does this mean for me?",
    "How long do I have to respond?",
    "Can I ignore this?",
    isUK ? "What if I can\'t afford a solicitor?" :
    isUS ? "What if I can\'t afford a lawyer?" :
...
function CountryBadge({ countryCode, onPress }) {
  ...
  return (
    <button onClick={onPress} style={{
      ...
      <span style={{ fontFamily: SANS, fontSize: 12, color: C.text2, fontWeight: 600, maxWidth: 80, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{country.name}</span>
...
function OnboardingScreen({ onDone }) {
    ...
    },
    {
      ...
      visual: (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {CATS.map(c => (
            <div key={c.id} style={{ background: C.surf, border: `1px solid ${C.border}`, borderRadius: 12, padding: "10px 8px", textAlign: "center" }}>
              <div style={{ marginBottom: 2 }}>{CAT_ICONS[c.id]?.(c.color)}</div>
              <div style={{ fontFamily: SANS, fontSize: 10, color: C.text2, marginTop: 4, lineHeight: 1.3 }}>{c.label}</div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: C.bg, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", justifyContent: "flex-end", padding: "calc(env(safe-area-inset-top, 0px) + 46px) 24px 0" }}>
...
function HomeTab({ usage, onSelectCat, history, onHistItem, countryCode, onChangeCountry }) {
  ...
  return (
    <div style={{ padding: "0 20px 140px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, paddingTop: "calc(env(safe-area-inset-top, 0px) + 46px)", paddingBottom: 28 }}>
            ...
            return (
              <div key={item.id} onClick={() => onHistItem(item)} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontFamily: SANS, fontSize: 12, color: C.text2 }}>{cat?.label}</span>
...
function ChatScreen({ catId, usage, onBack, onResult, onPaywall, countryCode }) {
  ...
  if (loading) {
    return (
      <div style={{ position: "fixed", inset: 0, background: C.bg, display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "calc(env(safe-area-inset-top, 0px) + 44px) 20px 18px", borderBottom: `1px solid ${C.border}`, background: C.surf, display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 24, height: 24, borderRadius: 6, background: C.surf2, animation: "pulse 1.5s ease-in-out infinite" }}/>
          <div style={{ width: 80, height: 16, borderRadius: 8, background: C.surf2, animation: "pulse 1.5s ease-in-out infinite" }}/>
        </div>
        <div style={{ flex: 1, padding: "28px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 8 }}>
            <WaythroughIcon size={36} glow />
            <div>
              <div style={{ fontFamily: SERIF, fontSize: 20, color: C.text, marginBottom: 4 }}>
                {file ? "Reading your document…" : "Analysing your situation…"}
              </div>
              <div style={{ fontFamily: SANS, fontSize: 13, color: C.text2 }}>This usually takes a few seconds</div>
            </div>
          </div>
          {[1, 0.7, 0.5, 0.8, 0.6].map((w, i) => (
            <div key={i} style={{
              height: i === 0 ? 80 : 20, borderRadius: 10,
              background: C.surf,
              width: `${w * 100}%`,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: C.bg, display: "flex", flexDirection: "column" }}>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "calc(env(safe-area-inset-top, 0px) + 44px) 20px 18px", borderBottom: `1px solid ${C.border}`, background: C.surf }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: C.text2, cursor: "pointer", padding: "4px 8px 4px 0", display: "flex", alignItems: "center" }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M15 18l-6-6 6-6" stroke={C.text2} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </button>
        <div style={{ flexShrink: 0 }}>{CAT_ICONS[cat.id]?.(cat.color)}</div>
        <div style={{ flex: 1 }}>
...
function ResultScreen({ catId, situation, result, onBack, onSave, saved, chatHistory, onNewChatMessage, countryCode }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: C.bg, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "calc(env(safe-area-inset-top, 0px) + 44px) 20px 16px", borderBottom: `1px solid ${C.border}`, background: C.surf, position: "sticky", top: 0, zIndex: 10 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: C.text2, cursor: "pointer", padding: "4px 8px 4px 0", display: "flex", alignItems: "center" }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M15 18l-6-6 6-6" stroke={C.text2} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </button>
        <div style={{ flexShrink: 0 }}>{CAT_ICONS[cat.id]?.(cat.color)}</div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "22px 20px 140px" }}>
        ...
        {/* Ad unit */}
        <div style={{ margin: "8px 0 16px", borderRadius: 12, overflow: "hidden", minHeight: 90, background: C.surf, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <ins className="adsbygoogle"
            style={{ display: "block", width: "100%", minHeight: 90 }}
            data-ad-client="ca-pub-3245990202909798"
            data-ad-slot="auto"
            data-ad-format="auto"
...
function HistoryTab({ history, onItem, onDelete, onClear }) {
  ...
  return (
    <div style={{ padding: "0 20px 140px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: "calc(env(safe-area-inset-top, 0px) + 46px)", paddingBottom: 22 }}>
      ...
      {history.map(item => {
        ...
        return (
          <div key={item.id} style={{ background: C.surf, border: `1px solid ${C.border}`, borderLeft: `3px solid ${cat?.color || C.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 10, display: "flex", alignItems: "flex-start", gap: 12 }}>
            ...
            <button onClick={() => onDelete(item.id)} style={{ background: "none", border: "none", color: C.text3, fontSize: 20, cursor: "pointer", padding: "0 8px", flexShrink: 0, lineHeight: 1 }}>×</button>
          </div>
        );
      })}
    </div>
...
function HistoryDetail({ item, onBack, onDelete }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: C.bg, display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "calc(env(safe-area-inset-top, 0px) + 44px) 20px 16px", borderBottom: `1px solid ${C.border}`, background: C.surf }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: C.text2, cursor: "pointer", padding: "4px 8px 4px 0", display: "flex", alignItems: "center" }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M15 18l-6-6 6-6" stroke={C.text2} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        </button>
        <div style={{ flexShrink: 0 }}>{cat && CAT_ICONS[cat.id]?.(cat.color)}</div>
        <span style={{ fontFamily: SANS, fontSize: 15, fontWeight: 700, color: C.text, flex: 1 }}>{cat?.label}</span>
...
/* ═══════════════════════════════════════════════════════
   SETTINGS TAB
═══════════════════════════════════════════════════════ */
function SettingsTab({ onClearHistory, countryCode, onChangeCountry, theme, setTheme }) {
  const [showCountryPicker, setShowCountryPicker] = useState(false);
  const country = getCountryConfig(countryCode);
  const rows = [
    { label: "Rate Waythrough ⭐",  action: () => alert("App store rating coming soon — thank you!"),                                        color: C.text   },
    { label: `Appearance: ${theme === "system" ? "System" : theme === "dark" ? "Dark" : "Light"}`, action: () => { const next = theme === "system" ? "dark" : theme === "dark" ? "light" : "system"; setTheme(next); document.documentElement.setAttribute("data-theme", next); }, color: C.text },
  ...
  return (
    <div style={{ padding: "0 20px 140px" }}>
      <div style={{ fontFamily: SERIF, fontSize: 28, color: C.text, paddingTop: "calc(env(safe-area-inset-top, 0px) + 46px)", paddingBottom: 24, fontWeight: 400 }}>Settings</div>
...
export default function App() {
  ...
  useEffect(() => {
    const handlePopState = () => {
      if (overlay) {
        setOverlay(null);
        if (overlay === "hist-detail") setHistItem(null);
      } else if (tab !== "home") {
        setTab("home");
      }
    };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: C.bg, overflowY: "auto" }}>
      ...
      {tab === "settings" && <SettingsTab onClearHistory={handleClearHistory} countryCode={countryCode} onChangeCountry={handleChangeCountry} theme={theme} setTheme={setTheme} />}
